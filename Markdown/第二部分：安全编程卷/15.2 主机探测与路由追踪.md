...
date: '2023-8-4 11:10'
description: >-
  Ping 使用 `Internet` 控制消息协议（`ICMP`）来测试主机之间的连接。当用户发送一个 `ping` 请求时，则对应的发送一个 `ICMP
  Echo` 请求消息到目标主机，并等待目标主机回复一个 `ICMP Echo`
  回应消息。如果目标主机接收到请求并且网络连接正常，则会返回一个回应消息，表示主机之间的网络连接是正常的。如果目标主机没有收到请求消息或网络连接不正常，则不会有回应消息返回。
tags:
  - 《灰帽黑客：攻守道》
  - 第二部分：安全编程卷
  - Socket
...
![](/image/1379525-20230505164228707-1153705300.png)
...
![](/image/1379525-20230505165156425-83616883.png)
...
![](/image/1379525-20230505170227432-427475171.png)